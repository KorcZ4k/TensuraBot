...
class PerformanceSafetyTests(unittest.TestCase):
    ...
    def test_concurrent_offloads_do_not_serialize(self):
        async def run():
            ...
            return await asyncio.gather(*(operation(i) for i in range(32)))
    ...
    def test_legacy_compatibility_wrappers_are_real_and_importable(self):
        expectations = {
            "comandos/RPG/luta_sync.py": ("Luta", "_criar_participante"),
            "comandos/RPG/magias_sync.py": ("Magias", "setup"),
            "comandos/RPG/status_sync.py": ("Status", "setup"),
        }
        for relative, symbols in expectations.items():
            path = ROOT / relative
            ...
            imported = {alias.asname or alias.name.split(".")[-1] for node in tree.body if isinstance(node, ast.ImportFrom) for alias in node.names}
            available = defined | imported
            for symbol in symbols:
                self.assertIn(symbol, available, f"{relative}: {symbol}")
