...
class Luta(commands.Cog):
    ...
    # Os antigos metodos decorados abaixo continuam como compatibilidade de
    # API interna, mas o setup desta extensao remove seus registros e instala
    # apenas os callbacks de modulo definidos apos a classe.
...
# -------------------- comandos publicos: todos pertencem a este modulo --------------------
...
async def setup(bot):
    # Um unico Cog fornece o motor/estado. Os comandos decorados do Cog sao
    # imediatamente removidos do bot e substituidos pelos callbacks de modulo,
    # eliminando definitivamente o problema de binding self/ctx.
    ...
    # Diagnostico de registro: se outro modulo sobrescrever a luta depois,
    # o log mostra imediatamente qual callback ficou efetivamente instalado.
    comando_luta = bot.get_command("luta")
    comando_monstros = comando_luta.get_command("monstros") if comando_luta else None
    comando_pve = comando_luta.get_command("pve") if comando_luta else None
    print(
        "[LUTA][REGISTRO]",
        f"luta={bool(comando_luta)}",
        f"monstros={bool(comando_monstros)}",
        f"pve={bool(comando_pve)}",
        f"callback_monstros={getattr(comando_monstros, 'callback', None)}",
        f"callback_pve={getattr(comando_pve, 'callback', None)}",
    )


# Compatibilidade usada por outros Cogs do RPG.
