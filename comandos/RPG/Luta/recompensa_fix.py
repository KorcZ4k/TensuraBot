"""Compatibilidade de recompensa sem monkey patch.

O motor efetivo calcula XP, Hunos e TP em hardening_final.Luta.
"""
...
async def _recompensar_async(self, combate):
    ...
    for i, jogador in enumerate(vivos):
        ...
        await luta_db.run_db(luta_db.db["Jogadores"].update_one, filtro, {"$inc": {"XP": ganho_xp, "TP": ganho_tp}})
        await luta_db.run_db(luta_db.db["Hunos"].update_one, filtro, {"$inc": {"carteira": ganho_hunos}}, upsert=True)
    return xp, hunos, tp


__all__ = ["_recompensar_async"]
